 ...
 */
export class RevokeUserInvite implements UseCaseInterface<void> {
  ...
  public async execute(params: { workspaceId: string; inviteId: string }): Promise<Result<void>> {
    ...
    try {
      const url = buildApiWorkspacePath(workspaceId, `/user-invites/${inviteId}`)
      await this.executeAuthenticatedRequest.executeDelete(url)
      return Result.ok()
    } catch (error) {
      return Result.fail(error instanceof Error ? error.message : "Failed to revoke user invite")
    }
  }
}
