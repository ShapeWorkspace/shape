...
import { BuildInitialWorkspaceKeyParamsFromExistingKey } from "./BuildInitialWorkspaceKeyParamsFromExistingKey"
...
/**
 * Registers an existing local workspace on the server using its current workspace key.
 *
 * This use case handles the full workflow for converting a local-only workspace
 * (created before signup) into a server-backed workspace:
 * 1. Builds initial workspace key params from the existing local key
 ...
 * 5. Persists the workspace key to IndexedDB scoped to the authenticated user
 * 6. Updates the workspace store with the registered workspace
 */
export class RegisterExistingWorkspaceWithServer implements UseCaseInterface<Workspace> {
  constructor(
    private readonly buildInitialWorkspaceKeyParamsFromExistingKey: BuildInitialWorkspaceKeyParamsFromExistingKey,
  ...
  async execute(
    ...
    // Step 1: Build initial workspace key params from existing key
    const initialKeyResult = this.buildInitialWorkspaceKeyParamsFromExistingKey.execute(
      workspaceId,
      identityKeys,
      workspaceKey
    )
    if (initialKeyResult.isFailed()) {
      return Result.fail(`Failed to build initial key params: ${initialKeyResult.getError()}`)
    }
    const initialKeyParams = initialKeyResult.getValue()
    ...
    if (responseResult.isFailed()) {
      return Result.fail(`Failed to register workspace on server: ${responseResult.getError()}`)
    }
    const response = responseResult.getValue()

    // Step 4: Security validation - ensure key is scoped to correct workspace
    if (workspaceKey.workspaceId !== request.id) {
      this.logger.error(
        `Workspace key scope mismatch: key ${workspaceKey.id} belongs to ${workspaceKey.workspaceId} but request targets ${request.id}`
      )
    ...
    if (response.workspace.uuid !== request.id) {
      ...
      return Result.fail("Workspace ID mismatch - aborting workspace registration")
    }
    ...
    // Step 6: Persist workspace key to IndexedDB scoped to the authenticated user
    // This replaces the anonymous user key with one scoped to the actual account
    await this.persistWorkspaceKey(workspaceKey, identityKeys)
  ...
  private async persistWorkspaceKey(key: DecryptedWorkspaceKey, identityKeys: IdentityKeys): Promise<void> {
    ...
    this.logger.debug(
      `Persisted workspace key ${key.id} for workspace ${key.workspaceId} (user: ${identityKeys.userId})`
    )
  }
}
