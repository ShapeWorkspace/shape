...
// Import individual functions from sodium namespace
// Note: These are accessed via the namespace because libsodium-wrappers-sumo
// populates functions only after the `ready` promise resolves
const { base64_variants, from_base64, from_hex, from_string, to_base64, to_hex, to_string } = sodium
 ...
 */
export function arrayBufferToString(arrayBuffer: ArrayBuffer | Uint8Array): string {
 ...
 * @param arrayBuffer
 */
export function arrayBufferToHexString(arrayBuffer: ArrayBuffer | Uint8Array): string {
  const bytes = arrayBuffer instanceof Uint8Array ? arrayBuffer : new Uint8Array(arrayBuffer)
  return to_hex(Buffer.from(bytes))
}
 ...
 * @param buffer
 */
export function arrayBufferToBase64(arrayBuffer: ArrayBuffer | Uint8Array): string {
  const bytes = arrayBuffer instanceof Uint8Array ? arrayBuffer : new Uint8Array(arrayBuffer)
  return to_base64(Buffer.from(bytes), SN_BASE64_VARIANT)
}
...
export function base32Decode(b32Input: string): ArrayBuffer {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    if (!RFC4648.includes(char)) {
      throw new Error(`Invalid RFC4648 char ${char} at index ${i}`)
    }
  }
  ...
  for (let i = 0; i < input.length; i++) {
    currentVal = (currentVal << 5) | RFC4648.indexOf(input.charAt(i))
    bitIdx += 5
...
/**
 * Validate that a value is a hex string of a specific byte length.
 * This protects cryptographic calls from malformed or truncated key material.
 */
export function isHexStringWithExpectedBytes(value: unknown, expectedBytes: number): value is HexString {
