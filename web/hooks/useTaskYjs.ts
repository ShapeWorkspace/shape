/**
 * useTaskYjs hook manages Yjs document lifecycle for block-based task descriptions.
 *
 * Responsibilities:
 * 1. Creates and manages Y.Doc instance for the task
 ...
 * 5. Queues tasks for search indexing after content changes
 */

import { useState, useEffect, useRef, useCallback, useMemo } from "react"
import * as Y from "yjs"
import { useEngineStore } from "../store/engine-store"
import { useTaskBlocks, useCreateTaskBlock } from "../store/queries/use-project-tasks"
...
export function useTaskYjs({
  taskId,
  projectId,
  title,
  createdAt,
  updatedAt,
}: UseTaskYjsOptions): UseTaskYjsResult {
  ...
  const createBlockMutation = useCreateTaskBlock()
  const { data: blocks, isLoading: blocksLoading, error: blocksError } = useTaskBlocks(projectId, taskId)

  const indexTaskBlocksForSearch = useCallback(async () => {
    if (!application) {
      return
    }
    ...
    const indexResult = await indexBlockEntity.execute(taskId)
    if (indexResult.isFailed()) {
      console.warn("Failed to index task blocks for search:", indexResult.getError())
    }
  }, [application, taskId, createdAt, updatedAt])
  ...
  const applyLoadedBlocksToYDoc = useCallback(async () => {
    ...
    const entityKey = getEntityKey(taskId)
    if (!entityKey) {
    ...
    try {
      ...
      setIsSynced(true)
      await indexTaskBlocksForSearch()
    } catch (err) {
      console.error("Failed to apply blocks:", err)
      setError("Failed to load task description")
    } finally {
      isApplyingRemoteRef.current = false
      setIsLoading(false)
    }
  }, [blocks, application, getEntityKey, taskId, ydoc, indexTaskBlocksForSearch])
  ...
  const handleSavePendingUpdates = useCallback(
    ...
    [application, projectId, taskId, createBlockMutation, indexTaskBlocksForSearch]
  )
  ...
  useEffect(() => {
    if (isSynced) {
      void indexTaskBlocksForSearch()
    }
  }, [title, createdAt, updatedAt, isSynced, indexTaskBlocksForSearch])
  ...
  }, [application, taskId, getEntityKey, ydoc, indexTaskBlocksForSearch])
