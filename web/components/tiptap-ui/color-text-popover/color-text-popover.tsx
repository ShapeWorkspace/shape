...
export interface RenderColorButtonProps extends ButtonProps {
  ...
  withLabel?: boolean
  onColorChanged?: ({ type, label, value }: { type: ColorType; label: string; value: string }) => void
}
...
export interface ColorGroupProps {
  type: ColorType
  colors: ColorItem[][]
  onColorSelected: ({ type, label, value }: { type: ColorType; label: string; value: string }) => void
  ...
  return colors.map((group, groupIndex) => (
    <ButtonGroup key={`${type}-group-${groupIndex}`} orientation="horizontal">
      {group.map((color, colorIndex) => {
        const itemIndex =
          startIndexOffset + colors.slice(0, groupIndex).reduce((acc, g) => acc + g.length, 0) + colorIndex
...
export interface ColorTextPopoverProps extends Omit<ButtonProps, "type">, UseColorTextPopoverConfig {}
 ...
 */
export const ColorTextPopover = forwardRef<HTMLButtonElement, ColorTextPopoverProps>(
  (
    {
    ...
    const { isVisible, canToggle, activeTextStyle, activeHighlight, handleColorChanged, label, Icon } =
      useColorTextPopover({
        editor,
        hideWhenUnavailable,
        onColorChanged,
