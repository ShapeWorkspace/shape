...
export function useImageUpload(config?: UseImageUploadConfig) {
  ...
  const canInsert = Boolean(application?.isWorkspaceRemote()) && canInsertImage(editor)
  const isActive = isImageActive(editor)

  useEffect(() => {
    if (!editor || !application?.isWorkspaceRemote()) {
      setIsVisible(false)
      return
    }
  ...
  }, [editor, hideWhenUnavailable, application])

  const handleImage = useCallback(() => {
    if (!editor || !application?.isWorkspaceRemote()) return false
