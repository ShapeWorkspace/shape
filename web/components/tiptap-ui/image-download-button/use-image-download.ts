...
export interface UseImageDownloadConfig {
  ...
  /**
   * Optional function to resolve encrypted file IDs to blob URLs.
   * Used when images are stored encrypted and need to be decrypted before download.
   */
  resolveFileId?: (fileId: string) => Promise<string | null>
  /**
...
export async function downloadSelectedImage(
  ...
  // For encrypted images, we need to resolve fileId to get a downloadable URL
  // Check if src is valid (not null, empty, or a stale blob URL)
  const hasValidSrc = imageData?.src && !imageData.src.startsWith("blob:")

  if (!hasValidSrc && !imageData?.fileId) {
    console.error("No valid src or fileId for image download")
    return false
  }
  ...
  try {
    ...
    // If we have a fileId and a resolver, use it to get the blob URL
    if (imageData?.fileId && resolveFileId) {
      resolvedUrl = await resolveFileId(imageData.fileId)
      if (!resolvedUrl) {
        console.error("Failed to resolve fileId to URL")
        return false
      }
    } else if (hasValidSrc) {
      // Use the src URL directly or resolve it
      resolvedUrl = imageData!.src!
      if (resolveFileUrl) {
        resolvedUrl = await resolveFileUrl(resolvedUrl)
      }
    }
    ...
    // For blob URLs (from fileId resolution), skip sanitization as they're local
    let sanitizedUrl = resolvedUrl
    if (!resolvedUrl.startsWith("blob:")) {
      const baseUrl = window.location.href
      sanitizedUrl = sanitizeUrl(resolvedUrl, baseUrl)
    ...
    // Fallback: try to open the original src in a new tab
    if (imageData?.src) {
      try {
        const baseUrl = window.location.href
...
export function useImageDownload(config?: UseImageDownloadConfig) {
  ...
  const handleDownload = useCallback(async () => {
    ...
    return success
  }, [editor, onDownloaded, resolveFileUrl, downloadMethod, resolveFileId])
